...
export default function PackagesPage({ params }: Props) {
  const { tenantId, siteId } = use(params)
  ...
  useEffect(() => {
    import('@/lib/services/package.service').then(({ packageService }) =>
      packageService.getBySiteId(siteId).then((data) => {
        setPackages(data)
        setLoading(false)
      })
    )
